...
export default class PostRepo implements IPostRepo {
  ...
  public async save(post: Post): Promise<Post> {
    ...
    try {
      if (postDocument === null) {
        ...
        let oldReactions = Array.from(postDocument.reactions);

        postDocument.reactions = post.reactions ? await Promise.all(post.reactions.map(async (reaction) => {
        ...
        for (let oldReaction of oldReactions) {
          if (!postDocument.reactions.find(reaction => reaction == oldReaction)) {
            //await this.reactionSchema.deleteOne({_id: oldReaction})
          }
        }


        
        postDocument.comments = post.comments ?  await Promise.all(post.comments.map(async (comment) => {
          ...
          if (commentRecord != null) {
            let oldCommentReactions = Array.from(commentRecord.reactions)
            commentRecord.reactions = comment.reactions ? await Promise.all(comment.reactions.map(async (reaction) => {
              const reactionrecord = await this.reactionSchema.findOne({ domainId: reaction.id.toString() } as FilterQuery<IReactionPersistence & Document>);
              if (reactionrecord != null) {
                return reactionrecord._id;
              } else {
            ...
            for (let oldReaction of oldCommentReactions) {
              if (!commentRecord.reactions.find(reaction => reaction == oldReaction)) {
                //await this.reactionSchema.deleteOne({_id: oldReaction})
              }
            }
  ...
  public async findByDomainId(postId: PostId | string): Promise<Post> {
    ...
    const postRecord = await this.postSchema.findOne(query as FilterQuery<IPostPersistence & Document>).populate({path: "comments", populate: {path: "reactions"}}).populate("reactions");
    if (postRecord != null) {
  ...
  public async findPostsByPlayerId(playerId: string): Promise<Post[]> {
    ...
    const postRecords = await this.postSchema.find(query as FilterQuery<IPostPersistence & Document>).populate({path: "comments", populate: {path: "reactions"}}).populate("reactions");
    if (postRecords != null) {
